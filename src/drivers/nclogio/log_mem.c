 ...
 */
int nclogio_log_buffer_init(NC_Log_buffer * bp){
 ...
 */
void nclogio_log_buffer_free(NC_Log_buffer * bp){
    NCI_Free(bp->buffer);
}
 ...
 */
char* nclogio_log_buffer_alloc(NC_Log_buffer *bp, size_t size) {
    char* ret;
 ...
 */
int nclogio_log_sizearray_init(NC_Log_sizearray *sp){
    sp->values = (size_t*)NCI_Malloc(LOG_ARRAY_SIZE * sizeof(NC_Log_metadataentry*));
    if (sp->values == NULL){
        DEBUG_RETURN_ERROR(NC_ENOMEM);
    }
 ...
 */
void nclogio_log_sizearray_free(NC_Log_sizearray *sp){
    NCI_Free(sp->values);
}
 ...
 */
int nclogio_log_sizearray_append(NC_Log_sizearray *sp, size_t size) {
    size_t *ret;
