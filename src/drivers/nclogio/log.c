 ...
 */
int ncmpii_log_create(NC_Log* nclogp, MPI_Info info) {
    ...
    abspath = realpath(nclogp->path, basename);
    if (abspath == NULL){
        /* Can not resolve absolute path */
        DEBUG_RETURN_ERROR(NC_EBAD_FILE);
    }
    abspath = realpath(nclogp->logbase, logbase);
    if (abspath == NULL){
        /* Can not resolve absolute path */
        DEBUG_RETURN_ERROR(NC_EBAD_FILE);
    }
     ...
     */
    sprintf(nclogp->metalogpath, "%s%s_%d_%d.meta", logbase, fname, nclogp->ncp->ncid, rank);
    sprintf(nclogp->datalogpath, "%s%s_%d_%d.data", logbase, fname, nclogp->ncp->ncid, rank);
    ...
    nclogp->recdimid = -1;
    for(i = 0; i < nclogp->ncp->dims.ndefined; i++) {
        if (nclogp->ncp->dims.value[i]->size == NC_UNLIMITED){
            nclogp->recdimid = i;
            break;
        }
    }
    ...
    headerp->max_ndims = 0;    
    for(i = 0; i < nclogp->ncp->vars.ndefined; i++){
        if (nclogp->ncp->vars.value[i]->ndims > headerp->max_ndims){
            headerp->max_ndims = nclogp->ncp->vars.value[i]->ndims; 
        }
    }
...
#ifdef PNETCDF_DEBUG 
    if (nclogp->hints & NC_LOG_HINT_LOG_CHECK) {
        err = ncmpii_log_check_header(nclogp, 0);
        if (err != NC_NOERR){
            printf("******************************************************\n");
...
int ncmpii_log_enddef(NC_Log *nclogp){   
    ...
    for(i = 0; i < nclogp->ncp->vars.ndefined; i++){
        if (nclogp->ncp->vars.value[i]->ndims > headerp->max_ndims){
            maxdims = nclogp->ncp->vars.value[i]->ndims; 
        }
    }
...
int ncmpii_log_close(NC_Log *nclogp) {
    ...
    if (nclogp->metalog_fd >= 0){
        ...
        if (headerp->num_entries > 0){
            nclogp->ncp->logflushing = 1;
            log_flush(ncp);
            nclogp->ncp->logflushing = 0;
        }
        ...
        if (nclogp->ncp->hints & NC_LOG_HINT_DEL_ON_CLOSE){
    ...
    MPI_Reduce(&nclogp->total_time, &total_time, 1, MPI_DOUBLE, MPI_MAX, 0, nclogp->ncp->comm);
    MPI_Reduce(&nclogp->flush_read_time, &flush_read_time, 1, MPI_DOUBLE, MPI_MAX, 0, nclogp->ncp->comm);
    MPI_Reduce(&nclogp->flush_replay_time, &flush_replay_time, 1, MPI_DOUBLE, MPI_MAX, 0, nclogp->ncp->comm);
    MPI_Reduce(&nclogp->flush_total_time, &flush_total_time, 1, MPI_DOUBLE, MPI_MAX, 0, nclogp->ncp->comm);
    MPI_Reduce(&nclogp->log_write_time, &log_write_time, 1, MPI_DOUBLE, MPI_MAX, 0, nclogp->ncp->comm);
    MPI_Reduce(&nclogp->log_total_time, &log_total_time, 1, MPI_DOUBLE, MPI_MAX, 0, nclogp->ncp->comm);
    MPI_Reduce(&nclogp->total_meta, &total_meta, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM, 0, nclogp->ncp->comm);
    MPI_Reduce(&nclogp->total_data, &total_data, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM, 0, nclogp->ncp->comm);
 ...
 */
int ncmpii_log_put_var(NC_Log *nclogp, NC_var *varp, const MPI_Offset start[], const MPI_Offset count[], const MPI_Offset stride[], void *buf, MPI_Datatype buftype, MPI_Offset packedsize){
    int err, varid, dim;
...
#ifdef PNETCDF_DEBUG
    if (nclogp->ncp->hints & NC_LOG_HINT_LOG_CHECK) {
        if (stride == NULL){
...
int ncmpii_log_flush(NC_Log* nclogp) {
    ...
    nclogp->ncp->logflushing = 1;
    numput = nclogp->ncp->numPutReqs;
    putlist = nclogp->ncp->put_list;
    nclogp->ncp->numPutReqs = 0;
    nclogp->ncp->put_list = NULL;
    ...
    nclogp->ncp->logflushing = 0;
    NCI_Free(nclogp->ncp->put_list);
    nclogp->ncp->numPutReqs = numput;
    nclogp->ncp->put_list = putlist;
    ...
    /*
    if (nclogp->total_data != nclogp->ncp->put_size){
        printf("Warning: putsize mismatch after flush. log: %llu, nc: %llu\n", nclogp->total_data, nclogp->ncp->put_size);
    }
    */
#endif
...
int ncmpii_log_inq_put_size(NC_Log *nclogp, MPI_Offset *putsize){
    *putsize = nclogp->ncp->put_size + (MPI_Offset)nclogp->datalogsize - 8;
    return NC_NOERR;
}

int ncmpii_log_inq_num_recs(NC_Log *nclogp, MPI_Offset *numrecs){
