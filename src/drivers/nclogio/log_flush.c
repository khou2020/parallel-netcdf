 ...
 */
int split_iput(NC_Log *nclogp, NC_var *varp, MPI_Offset *start, MPI_Offset *count, MPI_Offset *stride, MPI_Datatype buftype, MPI_Offset dataoff, MPI_Offset datalen, size_t buffersize, void *buffer) {
    int i, err;
...
int log_flush(NC_Log *nclogp) {
    ...
    if (nclogp->ncp->logflushbuffersize > 0 && databuffersize > nclogp->ncp->logflushbuffersize){
        databuffersize = nclogp->ncp->logflushbuffersize;
    }
    /* Allocate buffer */
