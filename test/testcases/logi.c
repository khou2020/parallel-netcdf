...
int simpletest(char* fname, int enable_log){
	...
	if (MyRank < MAXPROCESSES) {
		...
		start[0] = MyRank;
		start[1] = 0;
		ret = ncmpi_iput_vara_int(fid, vid, start, count, buffer, &req);
		if (ret != NC_NOERR) {
		...
		ncmpi_wait(fid, NC_REQ_ALL, NULL, NULL);
		if (ret != NC_NOERR) {
			printf("Error wait: %d", ret);
			goto ERROR;
		}
	...
	if (MyRank < MAXPROCESSES) {
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < NProc; j++) {
				start[0][0] = MyRank;
				start[0][1] = i * NProc + j;
				ret = ncmpi_iput_var1_int(fid, vid[i], start[0], buffer, &req);
				if (ret != NC_NOERR) {
		...
		for (i = 0; i < 4; i++) {
			...
			count[0][0] = NProc;
			count[0][1] = 1;
			ret = ncmpi_iput_vara_int(fid, vid[i], start[0], count[0], buffer, &req);
			if (ret != NC_NOERR) {
		...
		for (i = 0; i < 4; i++) {
			...
			stride[0] = 1;
			stride[1] = 2;
			ret = ncmpi_iput_vars_int(fid, vid[i], start[0], count[0], stride, buffer, &req);
			if (ret != NC_NOERR) {
		...
		for (j = 0; j < 4; j++) {
			...
			ret = ncmpi_iput_varn_int(fid, vid[j], NProc, sp, cp, buffer, &req);
			if (ret != NC_NOERR) {
		...
		// Wait for non-blocking request
		ret = ncmpi_wait_all(fid, NC_REQ_ALL, NULL, NULL);      
		if (ret != NC_NOERR) {
			printf("Error wait: %d\n", ret);
			goto ERROR;
		}
